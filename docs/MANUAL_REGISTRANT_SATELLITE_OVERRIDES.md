...
Protected** by Satellite Settings and excluded from **Needs Review**, including
when the imported Satellite is missing. Dashboard and Satellite analytics count
the overridden Satellite and its Hub even if no imported Satellite link exists.
The manual assignment stays protected from synchronization, and the original
imported values remain preserved as source evidence.
